...
with open('perfect_isolated_heart_clips.json') as f:
...
for slot_idx, clip_val in clips_data['a4'].items():
    ...
    if match:
        print(f"Injecting isolated clipPath for {slot_id} ({len(clip_val)} chars)")
...
print("Successfully injected isolated heart clipPaths into js/modules/pl-globals.js!")
