...
def extract_canva_image_slots(svg_file, canvas_w, canvas_h):
    ...
    for elem in root.iter():
        ...
        # Get image's full matrix transform
        M_img = get_element_transform(elem, parent_map)
        
        for ctag, child in clip_defs[clip_id]:
            M_child = get_element_transform(child, parent_map)
            M_total = M_img @ M_child
            ...
            # Filter out whole page background image
...
# Extract
a4_canva_slots = extract_canva_image_slots('a5-1-new.svg', 2480, 3507)
a5_canva_slots = extract_canva_image_slots('a4-1-new.svg', 1748, 2480)

print(f"Extracted {len(a4_canva_slots)} Canva image slots from a5-1-new.svg for A4 template")
print(f"Extracted {len(a5_canva_slots)} Canva image slots from a4-1-new.svg for A5 template")

# Display extracted Canva slot positions
print("\n--- Canva A4 Slots (a5-1-new.svg) ---")
for i, s in enumerate(sorted(a4_canva_slots, key=lambda x: x['cy'])):
    print(f"  [{i}] cx={s['cx']:.1f}, cy={s['cy']:.1f}, w={s['w']:.1f}, h={s['h']:.1f}, ctag={s['ctag']}")

print("\n--- Canva A5 Slots (a4-1-new.svg) ---")
for i, s in enumerate(sorted(a5_canva_slots, key=lambda x: x['cy'])):
    print(f"  [{i}] cx={s['cx']:.1f}, cy={s['cy']:.1f}, w={s['w']:.1f}, h={s['h']:.1f}, ctag={s['ctag']}")
