...
with open('exact_multi_subpath_clips.json') as f:
...
for slot_idx, clip_val in clips_data['a4'].items():
    ...
    if match:
        print(f"Injecting exact multi clipPath for {slot_id} ({len(clip_val)} chars)")
...
print("Successfully injected exact multi clipPaths into js/modules/pl-globals.js!")
