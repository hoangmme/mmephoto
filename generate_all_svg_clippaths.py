...
def rect_to_svg_path(x, y, w, h, rx=0, ry=0):
    """Convert rect to path M...L...Z."""
    return f"M {x} {y} L {x+w} {y} L {x+w} {y+h} L {x} {y+h} Z"

def extract_svg_clip_shapes(svg_file, canvas_w, canvas_h):
    ...
    for elem in root.iter():
        ...
        for ctag, child in clip_defs[clip_id]:
            ...
            if ctag == 'rect':
                ...
                d_str = rect_to_svg_path(x, y, w, h)
            elif ctag == 'path':
                d_str = child.get('d', '')
            else:
                continue
                
            # Parse tokens and transform coords to canvas absolute
            ...
            while i < len(tokens):
                ...
                if cmd in ('M', 'L'):
                    ...
                    abs_coords.append((tx * scale_x, ty * scale_y, cmd))
                    i += 2
                elif cmd == 'C':
                    for _ in range(3):
                        x_pt, y_pt = float(tokens[i]), float(tokens[i+1])
                        tx, ty = transform_point(M_total, x_pt, y_pt)
                        abs_coords.append((tx * scale_x, ty * scale_y, 'C'))
                        i += 2
                else:
                    i += 1
            ...
            # Filter out tiny or full-page clipPaths
            if bw <= 30 or bh <= 30 or bw >= canvas_w * 0.95 or bh >= canvas_h * 0.95:
                continue
...
def generate_rel_clippath(shape, slot_cx, slot_cy):
    tokens = shape['tokens']
    M_total = shape['M_total']
    ...
    while i < len(tokens):
        ...
        if cmd in ('M', 'L'):
            x_pt, y_pt = float(tokens[i]), float(tokens[i+1])
            tx, ty = transform_point(M_total, x_pt, y_pt)
            rx = tx * scale_x - slot_cx
            ry = ty * scale_y - slot_cy
            c_name = 'M' if cmd == 'M' else 'L'
            parts.append(f"{c_name} {rx:.1f} {ry:.1f}")
            i += 2
        elif cmd == 'C':
            c_pts = []
            for _ in range(3):
                ...
                rx = tx * scale_x - slot_cx
                ry = ty * scale_y - slot_cy
                c_pts.extend([f"{rx:.1f}", f"{ry:.1f}"])
                i += 2
            parts.append(f"C {' '.join(c_pts)}")
        else:
            i += 1
...
# Extract shapes
a4_svg_shapes = extract_svg_clip_shapes('a5-1-new.svg', 2480, 3507)
a5_svg_shapes = extract_svg_clip_shapes('a4-1-new.svg', 1748, 2480)

print(f"Extracted {len(a4_svg_shapes)} valid cutout shapes from a5-1-new.svg for A4")
print(f"Extracted {len(a5_svg_shapes)} valid cutout shapes from a4-1-new.svg for A5")
